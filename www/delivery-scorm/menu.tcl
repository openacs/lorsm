...
db_foreach organizations {
    ...
    if { $debuglevel == 1 } { ns_log debug "Menu found organization $org_title"}
	...
	WHERE 
	   ...
	   AND
    	...
    	if { $debuglevel == 1 } {ns_log debug "Menu found item $ims_item_id $item_title"}
...
if { [info exists js] } {
    ...
    set TREE_HASH [join $TREE_HASH "\n"]
    	...
    	if { $debuglevel == 1 } {ns_log warning "delivery-scorm::menu.tcl select is not giving back a MENU !!!!"}
}
