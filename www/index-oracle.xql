...
--	   'Click here' as hasrtedata,   
	...
	from acs_objects acs, ims_cp_manifest_class cpmc, 
           lorsm_course_presentation_fmts pf, -- micheles
           ims_cp_manifests cp, (select man_id, max(scorm_type) as scorm_type from ims_cp_resources group by man_id ) cpr

	where cp.man_id = acs.object_id
	and cpr.man_id (+) = cp.man_id
