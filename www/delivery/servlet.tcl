ad_page_contract {
    ...
    Mail tcl procedures to handle colloquia with java rte applet
...
ns_log Notice "---------------------------------------"
ns_log Notice "		LMS Rte server START"
ns_log Notice "---------------------------------------"
ns_log Notice "username: $username"
set currentcourse [ad_get_client_property lorsm currentcourse]
ns_log Notice "man_id: $currentcourse"
...
ns_log Notice "item_id: $currentpage"

if { ! ($currentpage>0) } {
	ns_log $level "SCORM lorsm::get_ims_item_id is empty !!!!! ERRROR ERRROR ERROR $currentpage"
}
ns_log $level "SCORM la pagina restituit a da get ims item_id e' $currentpage"

if { ! ($currentpage>0) } {
	ns_log $level "SCORM currentpage empty !!!!! ERRROR ERRROR ERROR $currentpage"
}

ns_log $level "SCORM beginning transaction for user: $user_id course: $currentcourse page (ims_item_id): $currentpage"
...
ns_log $level "Applet beginning with parameters currenttrack: $currenttrackid lorsmtrack: $lorsmstudenttrack community_id: $community_id"
...
switch -regexp $functionCalled {
	...
	if { $lorsmstudenttrack == 0 || $lorsmstudenttrack == ""  } {
		...
		if { ! [ db_0or1row isanysuspendedsession "
			...
			ns_log $level "SCORM Tracking has generated a lorsm_student_track new track id: $currenttrackid"
		} else {
	    		ns_log $level "SCORM Tracking has not found a lorsm_cmi_core track with a non completed nor passed session."
			set currenttrackid $track_id
			set lorsmstudenttrack [lorsm::track::new \
			      	-user_id $user_id \
		      		-community_id $community_id \
		      		-course_id $currentcourse]
	    		ns_log $level "SCORM Tracking has in any case created a lorsm_student_tracking track_id: $lorsmstudenttrack"
...
set student_id "testscorm711"
		...
		# THIS CHECK is somehow just a previous bug catcher, shouldn't actually be needed
						if { [db_resultrows] == 1 } { 
							...
							ns_log Error "SCORM recoverying of student data: not successfull -> please check"
						} 

	        ns_log $level "SCORM retrieved track id in lorsm_cmi_core: $currenttrackid"
	...
	if { [ string length $session_time_ms] == 1 } { set session_time_ms "$prefix$session_time_ms" }
	...
	if { [ string length $total_time_ms] == 1 } { set total_time_ms "$prefix$total_time_ms" }
	if { [ string length $hours ] == 1 } { set hours "$prefix$hours" }
	if { [ string length $minutes] == 1 } { set minutes 0$minutes }
	if { [ string length $seconds] == 1 } { set seconds 0$seconds}
	...
	ns_log $level "SCORM passing data back to applet : $returndata"
	...
	{
	ns_log Notice "---------------------------------------"
	ns_log Notice "		LMSCommit/Finish"
	ns_log Notice "---------------------------------------"
	ns_log $level "SCORM received data $data from applet: processing. Reference cmi track is $currenttrackid, while lorsmstudenttrack is: $lorsmstudenttrack"
	...
	foreach couple $lista {
		...
		if { [ string length $value ] == 0 } {	
			ns_log debug "SCORM EMPTY settings : '$request' received and empty -> treating this applet anyway"
			} 
			#else { 
		...
		switch $functionCalled {
			cmiputcat {
				...
				ns_return 200 text/plain "OK"
				ns_log Notice "SCORM post LMSCommit (trackid=$currenttrackid)"
				} 
			cmiputcatONFINISH {
				set lorsmstudenttrack [ad_get_client_property lorsm studenttrack]
				...
				if { $lorsmstudenttrack == "" || $lorsmstudenttrack == 0 } {
					...
					#speficic for courses for which istrackable is on
        				lorsm::track::exit -track_id $lorsmstudenttrack } 
				ns_return 200 text/plain "OK"
				ns_log Notice "SCORM post LMSFinish (trackid=$currenttrackid) sent ok to applet"
				}
			}
	}
	default 
...
ns_log Notice "---------------------------------------"
ns_log Notice "		LMS Rte server END"
ns_log Notice "---------------------------------------"
