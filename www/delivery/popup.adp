<head>
    ...
    //<--!
        ...
        function LMSGetErrorString(number)  {
            debug ("sco called LMSGetErrorString ("+number+")"); return APIFinder.LMSGetErrorString(number)
        }

        function LMSGetDiagnostic(placeholder)  {
            debug ("sco called LMSGetDiagnostic ("+placeholder+")"); return APIFinder.LMSGetDiagnostic(placeholder)
        }
        ...
        function waitforapplet() {
            ...
            if (megatries>5) {
                ...
                if(bored==0) {
                    bored=1;
                    alert("Your browser seems not to let me communicate with the
                            applet. Please check:\n1) Java support is installed and
        ...
        function init() {
            ...
            //we use this weird setting not to have to set for typeof or undefined checking
            while ( findapplets != 1 && findapplets !=2 ) {
        ...
        function refreshFrame() {
            ...
            try {
                ...
                debug("Your browser told me the applet was ready but now denies
                        access to its functions, claiming \n"+err+"\n I must
                        force reload, this way you won't see this message again,
                        luckily.\n");
                return(0);
            }
            ...
            <if @menu_off@ lt 1>
                ...
                childwindow.frames['content'].document.location.href = "<if @ims_item_id@ defined>
                                                                            @body_url;noquote@
                                                                        </if>
                                                                        <else>
                                                                            body?man_id=@man_id@
                ...
                //let's show menu now
                childwindow.frames['menu'].document.location.href = "menu?man_id=@man_id@<if @ims_item_id@ defined>
                                                                                             &ims_id=@ims_item_id@
                                                                                        </if>
                                                                                            &track_id=@track_id@
                                                                                        <if @menu_off@ defined>
            ...
            <else>
                ...
                if (findgetAppletInfo> 7) {
                    debug("I cannot talk to APIAdapter: try reloading me. Please
                            notify support of your sw configuration ;) ");
                    return(0);
                }
           }
           getSize();
