...
if {![empty_string_p $imsitem_id]} {
    ...
    set man_id [db_string get_man_id "select man_id from ims_cp_items i, ims_cp_organizations o where i.ims_item_id = :imsitem_id and i.org_id = o.org_id"]

    set folder_id [db_string get_folder__id "select folder_id from ims_cp_manifests where man_id = :man_id"]

    # We display children
    db_1row item_info "select item_title, parent_item from ims_cp_items where ims_item_id = :imsitem_id"

    # Selected fields are renamed to avoid conflict with existing variables
    db_multirow -extend {href} children children {
	select 
	  ims_cp_items.ims_item_id as child_item_id, 
	  ims_cp_items.item_title as child_title 
	from 
	  acs_objects,
	  ims_cp_items
	where 
	  acs_objects.object_id = ims_cp_items.ims_item_id and
	...
	if { [template::multirow size children] == 0 } {
		...
		db_0or1row grandparent {
		    ...
		    where
		      ims_cp_items.ims_item_id = :parent_item and
		      ims_cp_items.parent_item = ims_cp_items_to_resources.ims_item_id and
