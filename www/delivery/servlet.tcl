...
ns_log $basiclevel "SCORM user: $user_id course: $currentcourse ims_item_id: $currentpage community_id: $community_id"
...
switch -regexp $functionCalled {
	...
	if { $lorsmstudenttrack == 0 || $lorsmstudenttrack == ""  } {
	    ...
	    if { ! [ db_0or1row isanysuspendedsession "
						select lorsm.track_id as track_id from 
	...
	if { ! [ db_0or1row istherealready "select * from lorsm_cmi_core where track_id = :currenttrackid "]} {
	    ...
	    #
		db_dml lmsinitialize { insert into lorsm_cmi_core(track_id,man_id,item_id,student_id,student_name,
								lesson_location,
							      lesson_status,
							      launch_data,
		...
		db_0or1row istherealready "select * from lorsm_cmi_core where track_id = :currenttrackid"
...
#}
		...
		if { ! [db_0or1row get_adlcp_student_data " select max_time_allowed ,time_limit_action ,mastery_score from lorsm_cmi_student_data where track_id=:currenttrackid "] } {
		...
		core  {
			...
			if { [ string first "time" $column ] >0  } { 
			    ...
			    regsub {^0} [lindex [split $value .] 1] "" msecs
							if { [empty_string_p $hours] } { set hours 0 }
							if { [empty_string_p $minutes] } { set minutes 0 }
							if { [empty_string_p $seconds] } { set seconds 0 }
							if { [empty_string_p $msecs] } { set msecs 0 }
